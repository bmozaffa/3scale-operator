...
func NewImages(options []string) Adapter {
	return NewAppenderAdapter(&Images{})
}

func (a *Images) Parameters() []templatev1.Parameter {
...
func (a *Images) Objects() ([]runtime.RawExtension, error) {
	imagesOptions, err := options()
	if err != nil {
		return nil, err
	}
	apicastComponent := component.NewApicast(imagesOptions)
